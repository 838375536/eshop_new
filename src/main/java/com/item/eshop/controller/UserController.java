...
//            @ApiImplicitParam(name = "school", value = "学校", required = true ,dataType = "String",paramType = "query")})
    ...
    public User login(@RequestParam(value = "phone")String phone, @RequestParam(value = "password")String password, HttpSession httpSession) {
        ...
        if(user!=null){
            ...
            if(user.getStatus()>=8){
                Shop shop = shopService.selectByUser(user.getId());
                sid = shop.getId();
                httpSession.setAttribute("shopId",shop.getId());
            }
            httpSession.setMaxInactiveInterval(2000000);
        }
        user.setOther(sid+"");
    ...
    @ApiOperation(value="用户修改个人信息",notes="返回值(User对象)或null值")
    @ApiImplicitParams({@ApiImplicitParam(name = "id", value = "用户id值", required = true ,dataType = "Integer",paramType = "query")})
    @PostMapping("/updateInfo")
    public User updateUserInfo(@RequestParam(value = "username",required = false)String username,@RequestParam(value = "avatar",required = false) MultipartFile avatar,HttpSession httpSession) {
        int myid = (int) httpSession.getAttribute("user_id");
        User user = new User();
        user.setUsername(username);
        if(avatar!=null) {
            String url = new UploadImage().uploadImage(avatar,0,constant.getPath(),constant.getUrl());
            user.setAvatar(url);
        }
        int res = userService.updateUser(user);
        return userMapper.selectByPrimaryKey(myid);
    }
    ...
    public void online(HttpSession httpSession) {
        ...
        if(userId!=null) {
            ...
            Cache.addUserIds(userId.toString());
        }
    }
    ...
    public void offline(HttpSession httpSession) {
        ...
        if(userId!=null) {
            ...
            Cache.removeUserIds(userId.toString());
        }
    }
